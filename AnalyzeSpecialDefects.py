...
if __name__ == '__main__' :
    ...
    entity_name = "Defect"
    # ident_query = 'Defect.Discussion = "%s"' % ident
    # ident_query = 'Defect.Discussion = "%s"' % ident
    response = rallyUtil.rally.get(entity_name, fetch=True,
                         workspace=workspace, project=project)

    for defect in response:

        # ident_query = "Defect.Discussion = %s" % defect.Discussion._ref
        ident_query = "Artifact.[_ref] = %s" % defect._ref
        response = rallyUtil.rally.get('ConversationPost', fetch=True,  query=ident_query,
                                       workspace=workspace, project=project)
    #
    ...
    #     rallyUtil.create_testcaseFromDefect( defectID, owner = None, **testcase_json)
